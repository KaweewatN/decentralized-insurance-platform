...
/**
 * A service for handling digital signatures for blockchain transactions.
 */
export class SignatureService {
  ...
  constructor() {
    const rpcUrl = process.env.SEPOLIA_RPC || process.env.ETH_RPC_URL;
    const privateKey = process.env.ADMIN_PRIVATE_KEY;
    this.adminPublicKey =
      process.env.ADMIN_PUBLIC_KEY || '0xYourAdminAddressHere';

    if (!rpcUrl || !privateKey) {
      throw new Error(
        '❌ Missing environment variables: RPC_URL, ADMIN_PRIVATE_KEY',
      );
    }
    ...
    this.logger.log(
      `✅ SignatureService initialized with admin address: ${this.adminPublicKey.substring(0, 6)}...`,
    );
  }
