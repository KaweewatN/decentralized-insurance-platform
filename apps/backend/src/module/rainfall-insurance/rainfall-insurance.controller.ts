...
export class RainfallInsuranceController {
  ...
  async estimatePremium(
    ...
    if (
      ...
      return {
        error:
          'Invalid input: latitude, longitude, threshold, and coverageAmount must be numbers.',
      };
    }
    ...
    if (conditionType !== 'below' && conditionType !== 'above') {
      return {
        error:
          'Invalid conditionType. Must be either "below" (drought) or "above" (flood).',
      };
    }
  ...
  adminReview(@Body() body: { applicationId: number; approved: boolean }) {
    return this.rainfallService.reviewApplication(
      body.applicationId,
      body.approved,
    );
  }
