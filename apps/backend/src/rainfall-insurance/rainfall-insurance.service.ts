...
import { Wallet, AbiCoder, solidityPackedKeccak256, getBytes, parseEther } from 'ethers';



@Injectable()
export class RainfallService {
  private readonly NASA_API_URL = 'https://power.larc.nasa.gov/api/temporal/daily/point';
  ...
  reviewApplication(applicationId: number, approved: boolean) {
    ...
    if (approved) {
      ...
      return { message: 'Application approved and ready for signing', policy: application };
    } else {
  ...
  async fetchRainfall(
    ...
    try {
      const response: AxiosResponse<any> = await this.httpService.axiosRef.get(this.NASA_API_URL, { params });
      ...
      if (!dailyValues || Object.keys(dailyValues).length === 0) {
        console.warn(`⚠️ No rainfall data for ${start} to ${end} at (${lat}, ${lon})`);
        return null;
      }

      const total: number = (Object.values(dailyValues) as number[]).reduce((acc: number, val: number) => acc + val, 0);
  ...
  async assessRiskAndCalculatePremium(
    ...
    for (let i = 1; i <= this.YEARS_TO_ANALYZE; i++) {
      ...
      const rainfall = await this.fetchRainfall(decodedLat, decodedLon, startDate, endDate, year);

      if (rainfall !== null) {
        validYears++;
        console.log(`✅ Valid rainfall for year ${year}: ${rainfall.toFixed(2)} mm`);
    ...
    if (validYears < 3) {
      return { error: 'Insufficient rainfall data found for analysis. Try a longer period or different location.' };
    }
    ...
    if (probability === 0) {
      return {
        error: 'No risk detected. Please adjust your threshold, dates, or location.'
      };
    }
  ...
  async signApprovedPolicy(applicationId: number): Promise<any> {
    ...
    const messageHash = solidityPackedKeccak256(
      ['uint256', 'address', 'uint256', 'uint256', 'uint256', 'string', 'string', 'uint8', 'int256', 'int256'],
      [policyId, userAddress, coverageAmount, premium, threshold, startDate, endDate, conditionType, lat, lon]
    );
  ...
  async confirmPayment(
    ...
    try {
      ...
      if (
        ...
        return { message: 'Transaction does not match expected payment details.' };
      }
