...
async function main() {
  ...
  const balance = await provider.getBalance(process.env.RAINFALL_CONTRACT_ADDRESS!);
  ...
  const encodedResult = ethers.AbiCoder.defaultAbiCoder().encode(["bool"], [result]);
  ...
  try {
    const tx = await contract.handleOracleFulfillment(fakeRequestId, encodedResult, "0x");
    await tx.wait();
