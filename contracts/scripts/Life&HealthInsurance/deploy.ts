...
async function main() {
  ...
  try {
    ...
    const adminRole = await (lifeContract as any).ADMIN_ROLE();
    const lifeAdminTx = await (lifeContract as any).grantRole(
      adminRole,
      deployer.address
    );
    ...
    const healthAdminTx = await (healthContract as any).grantRole(
      adminRole,
      deployer.address
    );
